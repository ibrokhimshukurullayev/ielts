...
async function main() {
  ...
  if (existing) {
    await prisma.test.update({
      where: { slug: "reading-test-5-vol7" },
      data: { title: "Full Reading Test 5 – Vol 7", content },
    });
    console.log("Updated existing test:", existing.id);
  } else {
    const created = await prisma.test.create({
      data: { skill: "READING", slug: "reading-test-5-vol7", title: "Full Reading Test 5 – Vol 7", content },
    });
