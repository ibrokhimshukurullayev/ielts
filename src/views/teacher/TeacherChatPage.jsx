...
import { useGroup, useCreatePost } from "@/src/features/chat";
import { Avatar, PostCard } from "@/src/widgets";

function PostForm({ teacher }) {
  ...
  return (
    <main className="fade-page mx-auto max-w-4xl px-4 py-0 sm:px-6">
      <div className="flex min-h-screen gap-0 lg:gap-6">
        ...
        <div className="min-w-0 flex-1 py-6">
          ...
          <div className="mb-5 flex items-center gap-3 rounded-2xl bg-white px-5 py-4 shadow-[0_2px_12px_rgba(15,32,68,0.08)]">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{members.length} students · you are admin</p>
            </div>
          </div>

          {/* Post form */}
          <div className="mb-5">
            <PostForm teacher={currentUser} />
          </div>
          ...
          {!isLoading && posts.length === 0 && (
            <div className="rounded-2xl border-2 border-dashed border-slate-200 py-14 text-center">
              ...
              <p className="mt-3 text-sm font-semibold text-slate-400">No posts yet</p>
              <p className="mt-1 text-xs text-slate-300">Write the first message — students can leave comments</p>
            </div>
          )}
        ...
        <div className="hidden w-56 shrink-0 py-6 lg:block">
          <div className="sticky top-24 overflow-hidden rounded-2xl bg-white shadow-[0_2px_12px_rgba(15,32,68,0.08)]">
            ...
            {members.length === 0 ? (
              <p className="px-4 py-4 text-xs text-slate-400">No students yet</p>
            ) : (
